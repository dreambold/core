...
Route::get('user/localization/{locale}', 'LocalizationController@index');

Route::get('localization/{locale}/{full}', 'LocalizationController@index');
Route::get('user/localization/{locale}/{full}', 'LocalizationController@index');
// Route::get('locale/{locale}',function ($locale){
//     Sesstion::put('locale',$locale);
...
Auth::routes();
Route::get('user/email-verification', 'Auth\RegisterController@verifyEmail')->name('email.verify')->middleware('HasEmailVerification');
...
Route::group(['prefix' => 'user'], function () {
    ...
    Route::get('/enable2fa', 'Auth\VerificationController@enable2fa')->name('enable2fa')->middleware(['auth','emailVerification']);
    Route::post('/enable2fa', 'Auth\VerificationController@enable2faPost')->name('enable2faPost')->middleware(['auth','emailVerification']);


    Route::group(['middleware' => ['auth','2fa','emailVerification']], function() {

        Route::get('/disable2fa', 'Auth\VerificationController@disable2fa')->name('disable2fa');

        Route::post('/disable2fa', 'Auth\VerificationController@disable2faPost')->name('disable2faPost');

        Route::patch('/spin', 'FrontendController@spin')->name('home.spin');

        // Route::get('/exchange', 'FrontendController@exchange')->name('exchange')->middleware('2faLimitExchange');
        // Route::get('/buy', 'FrontendController@buy')->name('buy')->middleware('2faLimitBuy');
        // Route::get('/sell', 'FrontendController@sell')->name('sell');

        Route::post('/shoes/buy/physical/{id}', 'ShoesController@purchase')->name('buy.shoe.physical')->middleware('2faLimitBuy');
        Route::post('/shoes/buy/virtual/{id}', 'ShoesController@virtualPurchase')->name('buy.shoe.virtual')->middleware('2faLimitBuy');
        Route::post('/shoes/sell/virtual/{id}', 'ShoesController@virtualSell')->name('sell.shoe.virtual')->middleware('2faLimitBuy');

        Route::get('/home', 'HomeController@index')->name('home')->middleware('2faLimitUser');
        //user Deposit
...
Route::group(['prefix' => 'admin', 'middleware' => 'auth:admin'], function () {
    ...
    Route::get('/shoes/buy/physical', 'ShoesController@showPurchases')->name('buy.shoe.physical.show');
    Route::post('/shoes/buy/physical/{id}/confirm', 'ShoesController@confirmPurchase')->name('buy.shoe.physical.confirm');
    Route::get('/shoes/log/exchange', 'ShoesController@showLog')->name('shoes.log');
...
Route::post('/2fa', function () {
    if(!session()->get('last_non_auth_urls')){
        return redirect('');
    }
    return redirect(session()->get('last_non_auth_urls'));
})->name('2fa')->middleware('2fa');

// Route::post('/2fa')->name('2fa')->middleware('2fa', 'guest');

// Route::post('reset-time-limit','Auth\VerificationController@resetTimeLimit')->name('reset.time.limit')->middleware('auth');

// Route::post('reset-time-limit-user','Auth\VerificationController@resetTimeLimitUser')->name('reset.time.limit.user')->middleware('auth');
// Route::post('reset-time-limit-exchange','Auth\VerificationController@resetTimeLimitExchange')->name('reset.time.limit.exchange')->middleware('auth');
// Route::post('reset-time-limit-buy','Auth\VerificationController@resetTimeLimitBuy')->name('reset.time.limit.buy')->middleware('auth');
