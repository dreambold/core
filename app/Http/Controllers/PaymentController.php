...
class PaymentController extends Controller
{
    ...
    public function userDataUpdate($data)
    {
        ...
        if ($data->status == 0) {
            ...
            $user['balance'] = $user->balance + $data->amount + $data->discount;
            ...
            if($user->refer!=0)
            {
                ...
                if($user->bonus_flag==0){
                    $refer['balance'] = round(($refer->balance + 100),$gnl->decimal);
                    $refer->update();

                    Trx::create([
                        'user_id' => $refer->id,
                        'amount' => round(100,$gnl->decimal),
                        'main_amo' => round($refer->balance,$gnl->decimal),
                        'charge' => 0,
                        'type' => '+',
                        'reffer' => 1,
                        'currency_id' => 964,
                        'title' => 'User Sign Up Commission  from '. $user->username ,
                        'trx' => str_random(16)
                    ]);
                ...
                if($refer->level == 1)
                {
                    $commision = ( ( $data->amount + $data->discount ) * $gnl->level_one)/100;
                }elseif ($refer->level == 2){
                    $commision = ( ( $data->amount + $data->discount ) * $gnl->level_two)/100;
                }elseif ($refer->level == 3){
                    $commision = ( ( $data->amount + $data->discount ) * $gnl->level_three)/100;
                }
                $refer_commission = $commision;
            ...
            /* Decucting relevant currency */
            if( $data->currency_id == 1 ){
                $currency->available_balance = $currency->available_balance - convertDcoToBitcoin($data->amount + $data->discount) - convertDcoToBitcoin($refer_commission);
            }else{
                $currency->available_balance = $currency->available_balance - convertDco( ($data->amount + $data->discount + $refer_commission) , $currency->price );
            }

            $currency->update();

            /*Add currency_id here*/
            Trx::create([
                'currency_id' => $data->currency_id,
                'user_id' => $user->id,
            ...
            $txt = ( $data->amount + $data->discount )  . ' ' . $gnl->currency . ' Deposited Successfully Via ' . $data->gateway->name;

            /* Updating currency balance */
            $currency = Currency::find($data->currency_id);
            $currency->available_balance = $currency->available_balance - ( $data->amount + $data->discount );
            $currency->save();
    ...
    public function depositConfirm(Request $request)
    {
        ...
        elseif ($data->gateway_id == 501) {
            ...
            if ($data->btc_amo == 0 || $data->btc_wallet == "") {
                ...
                // /* Relevant currency balance deduction */
                // /* Only Create Wallet if currency balance is available in the exchange */
                // $user = User::find($data->user_id);
                // $currency = Currency::find($data->currency_id);
                // $refer_commission = 0;
                // $bonus = 0;
                ...
                //     if($user->bonus_flag==0){
                //         $bonus = 100;
                //         Currency::find(964)->decrement('available_balance',100);
                //     }

                //     if($refer->level == 1)
                //     {
                //         $refer_commission = ( ( $data->amount + $data->discount ) * $gnl->level_one)/100;
                //     }elseif ($refer->level == 2){
                //         $refer_commission = ( ( $data->amount + $data->discount ) * $gnl->level_two)/100;
                //     }elseif ($refer->level == 3){
                //         $refer_commission = ( ( $data->amount + $data->discount ) * $gnl->level_three)/100;
                //     }
                // }
    ...
    public function blockIpnBtc()
    {
        \Log::info("Cron job: blockIpnBtc controller was called!");
        ...
        return back()->with('success', 'Run successfully!');
    }
